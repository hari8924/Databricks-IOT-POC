...
class HistoryLoader():
    ...
    def load_date_lookup(self):
        ...
        spark.sql(f"""INSERT OVERWRITE TABLE{self.catelog}.{self.db_name}.date_lookup 
    ...
    def assert_count(self, table_name, expected_count):
        print(f"Validating record count in {table_name}...", end='')
        actual_count = spark.read.table(f"{self.catelog}.{self.db_name}.{table_name}").count()
    ...
    def validate(self):
        ...
        self.asset_count(f"date_lookup",365)
        print(f"Historical data load valiation completed in {int(time.time()) - start} seconds")

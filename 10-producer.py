...
class Producer():
    def __init__(self):
        ...
        self.testr_data_dir = self.Conf.base_dir_data + "/test_data"
    ...
    def produce(self, set_num):
        ...
        if set_num <= 10:
            self.bpm(set_num)
        print(f"Test data set {set_num} producer in{int(time.time()) - start} seconds")

    def validate_count(self, format, location, expected_count):
        ...
        print(f"Found {actual_count:,} /Expected {expected_count:,} records: Sucess")

    def validate(self, sets):
        import time
        start = int(time.time())
        print(f"\nValidating test data {sets} sets...")
        self.validate_count("csv", "registered_users_bz/1registered_users", 5 if sets == 1 else 10)
        self.validate_count("json","kafka_multiplex_bz/2-user_info", 7 if sets == 1 else 13)
        self.validate_count("json","kafka_multiplex_bz/3-bpm", sets *253801)
        self.validate_count("json","kafka_multiplex_bz/4-workout", 16 if sets == 1 else 32)
        self.validate_count("csv", "gym_logins_bz/5-gym_logins", 8 if sets ==1 else 16)
